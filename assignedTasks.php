...
session_start();
$userFullNames = $_SESSION['fullnames']; 
...
// SELECT DATA FROM DB OR newOfficer !=''   assigned_to =  '$firstName $middleName $lastName'  OR  newOfficer ='$firstName $middleName $lastName' 
$sql = "SELECT * FROM ticket_details WHERE assigned_to != '' AND actionOnTickets = 'Opened' ORDER BY assigned_time   DESC limit 4 ";
       ...
       $string = $stringCut;
            ...
            if ($actionOnTickets == "Opened" ){
             ...
             <ul class="dropdown-menu">
               ...
               <li id='.$ticket_no.' class="commentOnly" title="Click to edit ticket no: '.$ticket_no.'"  ><a class="dropdown-item" href="#" >Add Comment Only</a></li>
...
if (mysqli_num_rows($result) > 0)
{
 ...
 while($row2 = mysqli_fetch_assoc($countRecresult)) {
  ...
  $sql = "SELECT * FROM ticket_details WHERE assigned_to = '$userFullNames' OR newOfficer = '$userFullNames' AND actionOnTickets = 'Opened' ORDER BY assigned_time   DESC limit 4  ";
  $result = mysqli_query($conn, $sql);
...
$countRec = "SELECT *, COUNT(*) AS prior_units FROM ticket_details WHERE assigned_to = '$userFullNames' OR newOfficer = '$userFullNames' AND actionOnTickets = 'Opened' ORDER BY id DESC ";
  $countRecresult = mysqli_query($conn, $countRec);
  if (mysqli_num_rows($result) > 0)
  {
              // output data of each row
   ...
   $returnResults = mysqli_query($conn, $sql_stmt);
     while($res = mysqli_fetch_assoc($returnResults)){
      ...
      //newOfficer !=''  OR assigned_to != '' 
   $sql = "SELECT * FROM ticket_details WHERE  (assigned_to =  '$firstName $middleName $lastName'  OR  newOfficer ='$firstName $middleName $lastName') AND (assigned_to =  '$firstName $middleName $lastName'  OR  newOfficer ='$firstName $middleName $lastName')  AND actionOnTickets = 'Opened' ORDER BY assigned_time   DESC limit 6 ";
   $result = mysqli_query($conn, $sql);
    ...
    while($row = mysqli_fetch_assoc($result)) {
       ...
       $assigned_time = $row['assigned_time']; 
          ...
          $string = $stringCut;
           ...
           <td class="one">
           <p class="fw-bold mb-1">'.$reassigned_to.'</p> 
           </td>
 ...
 $countRec = "SELECT *, COUNT(*) AS prior_units FROM ticket_details WHERE  (assigned_to =  '$firstName $middleName $lastName'  OR  newOfficer ='$firstName $middleName $lastName') AND (assigned_to =  '$firstName $middleName $lastName'  OR  newOfficer ='$firstName $middleName $lastName')   AND actionOnTickets = 'Opened' ORDER BY id DESC ";
   $countRecresult = mysqli_query($conn, $countRec);
   if (mysqli_num_rows($result) > 0)
   {
               // output data of each row
