...
if (mysqli_num_rows($result) > 0)
     ...
     while($row = mysqli_fetch_assoc($result)) {
        ...
        $date_issued = date('M d Y', $date_issued); 
         ...
         //set up  time
 if ($timeOfIssue != 0 || $timeOfIssue == null){
  ...
  $updatedTimeStamp = "None";
    ...
    <td>
     ...
     <p class="fw-bold mb-1"><button id='.$ticket_no.' class="commentOnly feedback btn btn-primary p-1 text-success" title="Your Feedback Is Relevant">Feedback</button></p>
...
';
 ...
 // SELECT DATA FROM DB  	access_level 
 $sql = "SELECT * FROM ticket_details WHERE staff_email ='$email' OR assigned_to LIKE '%$assigned_To%' ORDER BY timeOfIssue     DESC limit $start_from, $limit ";
 $result = mysqli_query($conn, $sql);
...
// pagination code 
$query = mysqli_query($conn, "SELECT * FROM ticket_details  WHERE staff_email ='$email'  OR assigned_to LIKE '%$assigned_To%' ");
$total_records = mysqli_num_rows($query);
$total_pages = ceil($total_records/$limit);
$table .= '
<ul class="pagination pagination-md pagination-circle">';
...
// SELECT DATA FROM DB  	access_level 
$sql = "SELECT * FROM ticket_details WHERE staff_email ='$email'  OR assigned_to LIKE '%$assigned_To%' OR newOfficer LIKE '%$assigned_To%'  ORDER BY timeOfIssue   DESC limit $start_from, $limit ";
$result = mysqli_query($conn, $sql);
...
// pagination code staff_email ='$email'  OR assigned_to LIKE '%$firstName $middleName $lastName%'
$query = mysqli_query($conn, "SELECT * FROM ticket_details  WHERE staff_email ='$email'  OR assigned_to LIKE '%$assigned_To%' OR newOfficer LIKE '%$assigned_To%'  ");
$total_records = mysqli_num_rows($query);
$total_pages = ceil($total_records/$limit);
$table .= '
<ul class="pagination pagination-md pagination-circle">';
...
$table .= '</ul>';
        ...
        if ( $assigned_time == 0 && $assigned_time == null){
         $assigned_time = 'None'; 
        }else{
         $assigned_time = $updatedTimeStamp; 
